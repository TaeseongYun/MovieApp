...
class OrderByRatingMovieRecyclerViewHolder(context: Context?, parent: ViewGroup) :
    BaseRecyclerViewHolder<MovieResult>(
        context, parent,
        R.layout.recycler_view_detail
    ) {
    ...
    override fun View.onBind(item: MovieResult) {
        if (item.posterPath.isNotEmpty())
            movieBackgroundIMG
                .loadMovieBackground("https://image.tmdb.org/t/p/w342${item.posterPath}")
